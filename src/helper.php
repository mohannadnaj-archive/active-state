...
if (!function_exists('set_active')) {
    function set_active(...$params)
    {
        static $loaded = [];
        // default values
        static $defaults = ['class'=> 'active', 'return' => 'class'];
...
if (!function_exists('_set_active_check_state')) {
    function _set_active_check_state(&$params, &$loaded, &$defaults)
    {
        ...
        if ($loaded[$params[0]]['active'] == $params[1]) {
            ...
            // return the class we set before while the return type is class
            elseif ($loaded[$params[0]]['return'] == 'class') {
